...
client.on("ready",()=>{
  client.user.setActivity(`🔔 m!yardım - m!davet`);
...
const emmmmbed = new Discord.MessageEmbed()
  ...
  .addField(
    `Mesa BOT | Teşekkürler`,
    `**Selamlar, Ben İsmail (Mesa Bot'un Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`
  )
  .addField(
    `Mesa BOT | Prefix`,
    `**Mesa Botun Prefixi(ön eki) = \`m!\`\n\n Değiştirebilmek için \`m!prefix\` Yazabilirsiniz.**`
  )
  .addField(
    `Mesa BOT | Nasıl Kullanılır?`,
    `**Mesa botun tüm özelliklerinden yararlanabilmek için sadece \`m!yardım\` yazmanız yeterlidir.**`
  )
  .addField(
    `Mesa BOT| Linkler`,
    `**Sohbet Kanalına m!davet Yazmanız Yeterlidir**`
  )
  .setFooter(`Mesa BOT | Gelişmiş Türkçe Bot | 2021`)
  .setTimestamp();
...
client.on("guildDelete", guild => {
  ...
  client.channels.cache.get("851874301543317523").send(Crewembed);
});
...
client.on("guildMemberAdd", member => {
  ...
  let embed = new Discord.MessageEmbed()
    .setDescription(
      "> :inbox_tray:  **Sunucuya yeni katılan** **" +
...
client.on("ready", async () => {
  let botVoiceChannel = client.channels.cache.get("851877390237433866");
